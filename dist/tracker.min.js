(function (global, factory) {
	typeof exports === 'object' && typeof module !== 'undefined' ? factory(exports, require('@/constant')) :
	typeof define === 'function' && define.amd ? define(['exports', '@/constant'], factory) :
	(factory((global.tracker = {}),global.constant));
}(this, (function (exports,constant) { 'use strict';
	...
	function http(data) {
	  ...
	  return new Promise(function (resolve, reject) {
	    ...
	    var url = "".concat(constant.SERVER_URL, "?time=").concat(Date.now());
	...
	var config = {
	  pageTime: true,
	  //是否记录页面停留时间
	  env: constant.ENVIRONMENT.PRODUCTION,
	  console: true,
	  projectId: null,
	  token: null,
	  version: null,
	  domain: '',
	  sendType: constant.SEND_TYPE.UNLOAD,
	  //发送日志方式 (同步发,异步延迟发,关闭浏览器前发送)
	  delayTime: 1000,
	  //延迟发送的时间
	  ...
	  corssSubdomain: true,
	  //false 域名不同认作为两个用户
	  analyseScript: "../dist/analyse.min.js" //script tracker-key  config
	...
	function login(info) {
	  if (_typeof(info) === 'object' && info) {
	    userInfo = _objectSpread({}, userInfo, info);
	  } else {
	...
	function send(data) {
	  ...
	  if (sendType === constant.SEND_TYPE.UNLOAD) {
	    var oldData = JSON.parse(localStorage.getItem(constant.TRACKER_DATA_KEY) || '[]');
	    localStorage.setItem(constant.TRACKER_DATA_KEY, JSON.stringify(oldData.concat(data)));
	  } else if (sendType === constant.SEND_TYPE.SYNC) {
	    _sendToServer([data]);
	  } else if (sendType === constant.SEND_TYPE.ASYNC) {
	    allData.push(data);
	...
	function sendStorageData() {
	  var oldData = localStorage.getItem(constant.TRACKER_DATA_KEY);

	  if (oldData && oldData !== '[]') {
	    _sendToServer(oldData).then(function () {
	      localStorage.removeItem(constant.TRACKER_DATA_KEY);
	    });
	  }
	}
	...
	function _wrapperData(data, config) {
	  index++;
	  return _objectSpread({}, data, clientInfo(), trackerInfo, getUserInfo(), {
	    trackTime: Date.now(),
	    useServerTime: config.useServerTime,
	    identify: getCookie(config.identify || constant.TRACKER_IDENTIFY),
	  ...
	  _createClass(ActionTracker, [{
	    ...
	    value: function trackPage() {
	      ...
	      var data = _objectSpread({
	        actionType: constant.ACTION_TYPE.PAGE,
	        url: location.origin,
	        host: location.host,
	    ...
	    value: function track() {
	      ...
	      var data = _objectSpread({
	        actionType: constant.ACTION_TYPE.EVENT,
	        eventName: 'CLICK',
	        pageId: this.pageId,
	        url: location.origin,
	        host: location.host,
	...
	function () {
	  ...
	  _createClass(PageTimeTracker, [{
	    ...
	    value: function end() {
	      this.change();
	      var data = {
	        actionType: constant.ACTION_TYPE.TIME,
	        url: location.origin,
	        host: location.host,
	        path: location.pathname,
	        hash: location.hash,
	...
	var install = function install(conf) {
	  ...
	  if (config.sendType === constant.SEND_TYPE.UNLOAD) {
