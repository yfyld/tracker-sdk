(function (global, factory) {
    ...
    ***************************************************************************** */
    ...
    function _generateData(data, config) {
        ...
        var resutl = __assign(__assign(__assign({}, newData), pageInfo), { trackTime: Date.now(), id: uuid + '-' + index });
        //单页面设置referrer
    ...
    var PageTimeTracker = /** @class */ (function () {
        ...
        PageTimeTracker.prototype.toSend = function () {
            if (this.config.autoTrackPage || this.info.trackId) {
                var durationTime = this.endTime - this.startTime - this.totalInvalidTime;
                if (durationTime < 100 && this.info.trackId) {
                    // 手动发后100豪秒内不能自动change
                    return;
                }
                var data = __assign({ actionType: ACTION_TYPE.PAGE, startTime: this.startTime, endTime: this.endTime, trackId: this.info.trackId, durationTime: durationTime }, this.info);
                send(data);
                this.info = {};
            }
        };
    ...
    var install = function (conf) {
        ...
        // }
        // 页面时间start
        if (config.pageTime && !instance.info.trackId) {
            instance.start();
        }
        // 单页面应用routerchange
