(function (global, factory) {
    ...
    ***************************************************************************** */
    ...
    var isArguments$1 = baseIsArguments$1(function() { return arguments; }()) ? baseIsArguments$1 : function(value) {
      return isObjectLike$1(value) && hasOwnProperty$j.call(value, 'callee') &&
    ...
    // Fallback for data views, maps, sets, and weak maps in IE 11 and promises in Node.js < 6.
    if ((DataView$1 && getTag$1(new DataView$1(new ArrayBuffer(1))) != dataViewTag$7) ||
        (Map$1 && getTag$1(new Map$1) != mapTag$8) ||
        (Promise$2 && getTag$1(Promise$2.resolve()) != promiseTag$1) ||
        (Set$1 && getTag$1(new Set$1) != setTag$8) ||
        (WeakMap$1 && getTag$1(new WeakMap$1) != weakMapTag$5)) {
    ...
    var setReferrerInfo = function (info) {
        referrerInfo = __assign(__assign(__assign({}, referrerInfo), info), { url: location.origin, host: location.host, path: location.pathname, hash: location.hash, title: document.title || '' });
    };
    ...
    var SERVER_URL = 'http://frontlo-collection.qa.91jkys.com/log.gif';
    ...
    if (scriptDom) {
        ...
        if (newConfig) {
            config = __assign(__assign({}, config), JSON.parse(newConfig));
        }
    }
    function getConfig() {
        return config;
    ...
    function _wrapperData(data) {
        ...
        var config = getConfig();
        index++;
        return __assign(__assign(__assign(__assign({ data: data }, clientInfo()), trackerInfo), getUserInfo()), { projectId: config.projectId, version: config.version });
    }
    function _generateData(data, config) {
        index++;
        if (isObject_1(data.custom)) {
        ...
        var resutl = __assign(__assign(__assign({}, data), getReferrerInfo()), { trackTime: Date.now(), id: uuid + '-' + index });
        //单页面设置referrer
    ...
    var PageTimeTracker = /** @class */ (function () {
        ...
        PageTimeTracker.prototype.resetStart = function () {
            this.invalidEndTime = this.invalidStartTime = this.startTime = Date.now();
        };
        PageTimeTracker.prototype.end = function () {
            this.endTime = Date.now();
            var data = __assign({ actionType: ACTION_TYPE.PAGE, startTime: this.startTime, endTime: this.endTime, durationTime: this.endTime - this.startTime - this.totalInvalidTime }, this.info);
        ...
        PageTimeTracker.prototype.change = function () {
            ...
            var data = __assign({ actionType: ACTION_TYPE.PAGE, startTime: this.startTime, endTime: this.endTime, trackId: this.info.trackId, durationTime: this.endTime - this.startTime - this.totalInvalidTime }, this.info);
            if (this.config.autoTrackPage || this.info.trackId) {
                send(data);
                this.info = {};
            }
    ...
    var ActionTracker = /** @class */ (function () {
        ...
        ActionTracker.prototype.trackPerformance = function () {
            ...
            var data = __assign({ actionType: ACTION_TYPE.PERFORMANCE }, info);
            send(data);
        };
            ...
            var _loop_1 = function (target) {
                ...
                if (target.getAttribute('data-track') ||
                    ...
                    if (element.tagName === 'A' && element.href) {
                        //劫持a链接注入本页面的code
                        var pageCode = getReferrerInfo().pageCode;
    ...
    var trackDom = instance$2.trackDom;
    var trackPage$1 = instance$2.trackPage;
    var trackEvent$1 = instance$2.trackEvent;
    var trackView$1 = instance$2.trackView;
    var trackLink = instance$2.trackLink;
    var track$1 = instance$2.track;
