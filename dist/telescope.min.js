(function (global, factory) {
    ...
    ***************************************************************************** */
    ...
    //日志收集接口
    var SERVER_URL = 'http://frontlo-collection.qa.91jkys.com/log.gif'; //'http://test.qa.91jkys.com:9342/log.gif';
    ...
    function http(data, isAjax, isSendBeacon) {
        ...
        return new Promise(function (resolve) {
            ...
            var dataStr = data;
            var url = SERVER_URL + "?" + (config.store ? "store=" + config.store + "&" : '') + "time=" + Date.now();
    ...
    function sendSync(data) {
        _sendToServer([_generateData(data)]);
    }
    /**
     * 延迟发送
     * @param data
    ...
    function sendAsync(data) {
        ...
        var config = getConfig();
        allData.push(_generateData(data));
        clearTimeout(timer);
    ...
    function _wrapperData(data) {
        ...
        var config = getConfig();
        index++;
        return __assign(__assign(__assign(__assign({ items: data }, clientInfo()), libInfo), getUserInfo()), { projectId: config.projectId, version: config.version });
    }
    /**
     * 补充埋点信息
     * @param data
    ...
    function _generateData(data, config) {
        ...
        var resutl = __assign(__assign(__assign({}, newData), getPageInfo()), { trackTime: Date.now(), id: uuid + '-' + index });
        //单页面设置referrer
    ...
    var track = function (partical) {
        if (typeof partical === 'function') {
            return function (target, key, descriptor) {
                ...
                //   }, descriptor);
                // }
                return set_1(prop_1('value', descriptor), value, descriptor);
            };
        }
