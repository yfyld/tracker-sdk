  ...
  */
	...
	function () {
		...
		_createClass(ActionTracker, [{
			...
			key: "trackLink",
			value: function trackLink(linkDom, info) {
			...
			key: "trackDom",
			value: function trackDom(dom, info) {
				var trackInfo = {
					domId: dom.id,
				...
				if (track && track.search(/^\{[\S\s]*\}$/) >= 0) {
					trackInfo = _objectSpread({}, trackInfo, JSON.parse(track));
				} else {
					trackInfo.trackId = track || "";
				}

				if (info) {
					trackInfo = _objectSpread({}, trackInfo, info);
				}
